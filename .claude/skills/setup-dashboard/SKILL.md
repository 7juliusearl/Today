...
- **Slack** (only if `SLACK_ENABLED`) — this is an **activity feed** of @mentions, modeled on Slack's own Activity tab. It is NOT a list of unread channels; the Slack tools available have no unread/read-state API, and grouping by channel loses the "who pinged me and when" signal that makes this useful. Read the CURRENT `data/dashboard.js`'s `generatedAt` as a cutoff (fallback: 14 days ago if missing/unreadable — mentions are lower-volume than unreads, so a longer lookback is fine and helps a Monday morning show Friday afternoon pings). Look up `USER_SLACK_ID` once during setup (see below) and bake the literal ID into the task prompt. Then call the combined public+private search tool once with `query: "<@USER_SLACK_ID>"`, the cutoff as the top-level Unix-timestamp `after` param, `filters: "after:<cutoff date, one day earlier>"`, `content_types: "messages"`, `sort: "timestamp"`, `sort_dir: "desc"`, `limit: 25`, `include_bots: false`. Discard results at/before the exact cutoff, discard any message the user themselves sent (a self-mention is not activity), and discard any message whose text doesn't actually contain the literal `<@USER_SLACK_ID>` token (Slack's search treats `<@…>` as a fuzzy hit — filter the token in text explicitly). For each survivor build `{ channel (the `#channel-name` for public/private, or the sender's display name prefixed with `@` for a DM), from (sender's display name — fall back to their username if the display name is empty), preview (~180 chars, keep the mention marker but resolve it to `@FirstName` in the preview if you can from the profile lookup you already did), ts (ISO timestamp with correct local offset), permalink (the message's permalink URL from the search result, or null) }`. Cap at 10, sorted most-recent-first. Set `slack.connected: true` if the call succeeded (even with 0 results), `false` only if the call itself failed. If `SLACK_ENABLED` is false, always output `{ connected: false, items: [] }` without attempting any Slack call.

  To get `USER_SLACK_ID` for the search query and self-filter above: call `slack_read_user_profile` with no `user_id` argument (defaults to the current user) and read the `User ID` field from the result. Do this once during setup and bake the literal ID into the task prompt you write (it won't change).
