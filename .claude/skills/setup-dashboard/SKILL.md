...
Record: `SLACK_ENABLED` (true/false). If enabled, mention to the user: a coworker can leave them an encouraging sticky note by DMing them on Slack starting with 📌 (or `:pushpin:` or `note:` if the emoji doesn't come through — all three work) — it'll show up on the dashboard automatically after the next refresh. Several people's notes can be stuck up at once, each in its own spot.
...
- **Sticky notes** (only if `SLACK_ENABLED`, plural — support several at once) — a coworker can leave an encouraging note that shows up as a physical-looking sticky note on the page, and more than one can be stuck up at a time (each in its own spot), not just the latest one overall. Real-world testing showed people trigger this inconsistently — some type an actual 📌 emoji, some type the literal shortcode `:pushpin:` which their Slack client doesn't always auto-convert, some just guess and type `note:`. Accept all three. Run THREE Slack searches with the same combined public+private search tool (merge results, dedupe by timestamp): `keywords: ["📌"]`, `keywords: ["pushpin"]`, and `keywords: ["note"]` — each with `filters: "is:dm"`, `content_types: "messages"`, `sort: "timestamp"`, `sort_dir: "desc"`, `limit: 10`, `include_bots: false`. From the merged results, keep only messages that (a) start with one of 📌 / `:pushpin:` / `note:` (case-insensitive) once trimmed — discard ones where it just appears mid-message, (b) weren't sent by the user themself, (c) are within the last 14 days. From what survives, keep up to the 3 most recent DISTINCT notes: if the same sender left more than one, only keep their latest (it replaces their older one), but different senders' notes are all kept, up to 3 total, sorted by recency. For each, strip whichever leading marker it used and build `{ text, from (sender's display name), ts (ISO timestamp) }`. Output the array (0-3 entries) as `stickyNotes` — empty array if nothing survives or all three searches fail, never a placeholder. If `SLACK_ENABLED` is false, always output `stickyNotes: []`.
...
- **Output**: overwrite `data/dashboard.js` (the exact path under this repo, using the absolute path from Phase 0) with:
  ...
  window.DASHBOARD_DATA = {
    ...
    stickyNotes: [ { text, from, ts }, ... ] (0-3 entries, [] if none),
...
> 3. If it DOES exist: leave it in place for now (don't delete it yet — its continued existence is how the dashboard's front end knows a refresh is already in flight, so it can warn against a redundant second click instead of silently double-triggering). Read `<absolute path to ~/.claude/scheduled-tasks/refresh-dashboard-data/SKILL.md>` and carry out EXACTLY the steps it describes, in full, writing to the exact same `data/dashboard.js`. Once that file has been written (or even if a step failed partway through — never leave this flag file behind), delete `data/.refresh-requested` as the very last action. Don't print a chat summary — just update the file and finish.
...
- Hitting the Refresh button gets real fresh data within about 1–2 minutes — not instant (there's no way for a browser button to instantly trigger Claude Code directly; the on-demand checker task is the fastest bridge available), but much faster than waiting for the next scheduled slot. It reloads automatically once the new data is ready (or after ~2.5 min either way), so there's no need to click it again and check manually. If it's clicked again while one is already running — from the same tab, another tab, or after a manual page reload — the dashboard recognizes that and shows "Already refreshing" instead of firing a second, redundant request.
