...
Add that exact HTTPS redirect to the Asana app. Retain the native/OOB redirect while the private developer trial is in use. Enable `tasks:read` and `workspaces:read` scopes. Restrict app distribution to the intended workspace(s). An organization's app approval policy still applies.
